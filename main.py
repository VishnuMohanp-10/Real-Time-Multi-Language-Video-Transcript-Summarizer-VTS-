...
class YouTubeLiveProcessor:
    ...
    async def get_stream_url_direct(self, video_id: str, websocket: WebSocket) -> Optional[str]:
        """
        Extract the best audio stream URL using yt-dlp with multiple client strategies.
        Tries android → android_vr → ios → tv_embedded → mweb → web in order.
        ...
        # HLS audio-only format IDs for YouTube live streams
        ...
        for strategy in strategies:
            ...
            try:
                ...
                info = await asyncio.to_thread(
                    ydl.extract_info, watch_url, download=False
                )
                ...
                # Direct URL (single format selected)
                url = info.get("url")
                if url:
    ...
    def generate_detailed_error_message(self, video_id: str) -> str:
        """Generate a clean, actionable error message"""
        return (
            f"Could not access YouTube stream (ID: {video_id}).\n\n"
            "Most likely causes:\n"
            "- The stream has not started yet or has already ended\n"
            "- The video is private, age-restricted, or members-only\n"
            "- YouTube is rate-limiting the server\n\n"
            "Tips:\n"
            "- Make sure the stream is currently LIVE and publicly accessible\n"
    ...
    async def process_stream(self, url: str, websocket: WebSocket, forced_language: Optional[str] = None):
        ...
        try:
            # ── Extract clean 11-char video ID ───────────────────────────────
            # Handles: /live/ID?si=..., /watch?v=ID, youtu.be/ID?si=..., /embed/ID
            video_id = None
            id_patterns = [
            ...
            # Extract stream URL — tries 6 client strategies in order
            await websocket.send_json({
                "type": "status",
                "data": "Extracting stream URL, please wait...",
                "timestamp": datetime.now().isoformat()
            })
